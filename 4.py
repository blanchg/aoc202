...
def isvalid(d):
    for v in valids:
        keys = d.keys()
        valid = True
        if len(keys) == len(v) and set(v).issubset(keys):
            byr = int(d['byr'])
            ...
            if measure == 'cm':
                ...
                valid = False
            hcl = d['hcl']
            if rehcl.fullmatch(hcl) is None:
                print(f'hcl={hcl}')
                valid = False
            ecl = d['ecl']
            if ecl not in eyes:
                ...
                valid = False
            pid = d['pid']
            if repid.fullmatch(pid) is None:
                print(f'pid={pid}')
                valid = False
            if valid:
                return 1
...
print(parse(x[0]))
print(isvalid(parse(x[0])))

print(sum(map(lambda x: isvalid(parse(x)), x)))
