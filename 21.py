...
def run1():
    ...
    ingredients = set()
    products = []
    products_by_allergen = defaultdict(list)
    products_by_ingredient = defaultdict(list)
    for i, line in enumerate(input):
        ingredientstr, allergenstr = line.split('(')
        allergenstr = allergenstr[8:-1]
        product_ingredients = ingredientstr.strip().split(' ')
        product_allergens = [x.strip() for x in allergenstr.strip().split(',')]
        allergens.update(product_allergens)
        ingredients.update(product_ingredients)
        products.append((i, product_ingredients, product_allergens))
        for allergen in product_allergens:
            products_by_allergen[allergen].append(i)
        for product_ingredient in product_ingredients:
            products_by_ingredient[product_ingredient].append(i)

    to_resolve = deque(allergens)
    allergen_to_name = {}
    used_ingredient = []
    while to_resolve:
        allergen = to_resolve.popleft()
        resolved = False
        products_with_allergen = products_by_allergen[allergen]
        for product_a in products_with_allergen:
            prod_a = products[product_a]
            unique_ingredients = set(prod_a[1])
            for product_b in products_with_allergen:
                if product_a == product_b:
                    continue
                prod_b = products[product_b]
                unique_ingredients = unique_ingredients.intersection(set(prod_b[1]))
                if len(unique_ingredients) == 0:
                     break
            unique_ingredients = unique_ingredients.difference(used_ingredient)
            if len(unique_ingredients) == 1:
                name = unique_ingredients.pop()
                if name not in used_ingredient:
                    resolved = True
                    allergen_to_name[allergen] = name
                    used_ingredient.append(name)
                    break
        if not resolved:
            to_resolve.append(allergen)

    result = 0
    for ingredient in ingredients:
        if ingredient not in used_ingredient:
            result += len(products_by_ingredient[ingredient])

    x = ','.join([allergen_to_name[x] for x in sorted(allergen_to_name.keys())])
    print(x)
    return result
